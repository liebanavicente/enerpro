...
async function cargarDocumentos() {
  ...
  if (cuadrante) {
    ...
    if (cuadranteDiv) cuadranteDiv.innerHTML = '<div style="color:var(--muted);font-size:0.875rem">No hay cuadrante disponible este mes.</div>';
  }
}
...
async function subirMasivo() {
  ...
  try {
    ...
    ok.style.display = 'block';
    ok.textContent = '✓ Proceso completado: ' + ok_count + ' documentos subidos.' + (fail_count ? ' ' + fail_count + ' no encontrados por DNI.' : '');
